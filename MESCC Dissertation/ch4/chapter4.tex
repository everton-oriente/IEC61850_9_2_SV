...
\section{Setup of the Algorithm}

The development was carried out using two hardware platforms to deploy the programs for running the algorithm. One of these platforms is a Jetson Nano, and the other is a personal computer. The Jetson Nano was selected due to its ease of use, as it allows for straightforward setup of all the necessary tools to deploy the program. Additionally, an extra Ethernet card can be added using a USB-to-Ethernet adapter, which is truly plug-and-play. This feature is essential for the setup, as it requires two Ethernet interfaces.
